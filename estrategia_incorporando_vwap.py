...
class EstrategiaCrucesEMAs:
    ...
    def confirmar_señal_compra(self, fila):
        """
        Confirma señal de compra con RSI, MACD y VWAP
        """
        confirmaciones = []
        ...
        # Precio debe estar por encima del VWAP (ya confirmado en la señal)
        if fila['close'] > fila['vwap']:
            confirmaciones.append("VWAP_OK")
        
        # Confirmación adicional: precio significativamente por encima del VWAP
        if fila['precio_vs_vwap'] > 0.5:  # 0.5% por encima del VWAP
            confirmaciones.append("VWAP_FUERTE")
        
        return confirmaciones
    
    def confirmar_señal_venta(self, fila):
        """
        Confirma señal de venta con RSI, MACD y VWAP
        """
        confirmaciones = []
        ...
        # Precio debe estar por debajo del VWAP (ya confirmado en la señal)
        if fila['close'] < fila['vwap']:
            confirmaciones.append("VWAP_OK")
        
        # Confirmación adicional: precio significativamente por debajo del VWAP
        if fila['precio_vs_vwap'] < -0.5:  # 0.5% por debajo del VWAP
            confirmaciones.append("VWAP_FUERTE")
    ...
    def ejecutar_estrategia(self):
        ...
        for señal in señales[-5:]:  # Últimas 5 señales
            ...
            print(f"    VWAP: {señal['vwap']:.4f} ({señal['precio_vs_vwap']:+.2f}%)")
        ...
        print(f"  VWAP: {ultimo_row['vwap']:.4f}")
        print(f"  Precio vs VWAP: {ultimo_row['precio_vs_vwap']:+.2f}%")
        print(f"  Tendencia: {ultimo_row['tendencia']}")
        print(f"  RSI: {ultimo_row['rsi']:.2f}")
        
        # Análisis de posición relativa
        if ultimo_row['close'] > ultimo_row['vwap']:
            vwap_signal = "🟢 Por ENCIMA del VWAP (Alcista)"
        else:
            vwap_signal = "🔴 Por DEBAJO del VWAP (Bajista)"
        print(f"  Señal VWAP: {vwap_signal}")
        
        # Recomendación basada en VWAP + EMAs
        if (ultimo_row['ema_rapida'] > ultimo_row['ema_lenta'] and 
            ultimo_row['close'] > ultimo_row['vwap'] and 
            ultimo_row['precio_vs_vwap'] > 0.2):
            recomendacion = "🚀 FUERTE ALCISTA - Considerar COMPRA"
        elif (ultimo_row['ema_rapida'] < ultimo_row['ema_lenta'] and 
              ultimo_row['close'] < ultimo_row['vwap'] and 
              ultimo_row['precio_vs_vwap'] < -0.2):
            recomendacion = "📉 FUERTE BAJISTA - Considerar VENTA"
        else:
            recomendacion = "⚠️ NEUTRAL - Esperar confirmación"
